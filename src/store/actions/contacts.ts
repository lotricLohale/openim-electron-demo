...
export const setCurrentMember = (value: GroupMemberItem): ContactActionTypes => {
  return {
...
export const setGroupMemberLoading = (value: GroupMemberState): ContactActionTypes => {
  return {
...
export const setOrganizationInfo = (value: OrzInfoType): ContactActionTypes => {
  return {
...
export const resetContactStore = (): ContactActionTypes => {
  return { type: RESET_CONTACT_STORE };
};
...
export const getGroupMemberList = (options: GetGroupMemberParams, preList = [] as GroupMemberItem[]) => {
  return (dispatch: Dispatch) => {
    dispatch(
    ...
    im.getGroupMemberList(options).then((res) => {
      ...
      dispatch(setGroupMemberList([...preList, ...list]));
      dispatch(
        setGroupMemberLoading({
          loading: false,
          hasMore: list.length === options.count,
        })
      );
    });
  };
...
export const getCurrentMember = (groupID: string,selfID: string) => {
  return (dispatch: Dispatch) => {
    im.getGroupMembersInfo({groupID,userIDList:[selfID]})
      .then((res) => {
        dispatch(setCurrentMember(JSON.parse(res.data)[0] ?? {}));
      })
      .catch((err) => dispatch(setCurrentMember({} as GroupMemberItem)));
  };
};
...
export const getOrganizationInfo = (userID: string) => {
  return (dispatch: Dispatch) => {
    let tempArr: DepartmentItem[] = [];
    // im.getSubDepartment({ departmentID: "", offset: 0, count: 20 }).then(({ data }) => {
    //   tempArr = JSON.parse(data);
    //   im.getUserInDepartment(userID).then(({ data }) => {
    //     const tmpMemberArr: any = [];
    //     (JSON.parse(data) ?? []).forEach((item: any) => {
    //       tempArr.push(item.department);
    //       tmpMemberArr.push(item.member);
    //     });
    //     dispatch(
    //       setOrganizationInfo({
    //         deps: tempArr,
