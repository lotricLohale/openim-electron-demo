...
export default class OpenIMSDK extends Emitter {
  ...
  login(config: InitConfig) {
    return new Promise<WsResponse>((resolve, reject) => {
      ...
      const onClose = () => {
        ...
        if(this.loginFlag) {
          this.emit(CbEvents.ONWSCONNECTING,"" as any)
          this.reconnect();
        }
        reject(errData);
      };
  ...
  private wsSend = (params: WsParams, resolve: (value: WsResponse | PromiseLike<WsResponse>) => void, reject: (reason?: any) => void) => {
    ...
    //   return;
    // }
    if (this.ws?.readyState !== (WebSocket?.OPEN??0)) {
      let errData: WsResponse = {
        event: params.reqFuncName,
        errCode: 112,
        errMsg: "ws conecting...",
    ...
    const handleMessage = (ev: MessageEvent<string>) => {
      ...
      if (params.reqFuncName === RequestFunc.LOGOUT && this.ws2promise[data.operationID]) {
      ...
      if (!callbackJob) {
        if ((data.event === RequestFunc.SENDMESSAGE || data.event === RequestFunc.SENDMESSAGENOTOSS)&&data.errCode === 0) {
  ...
  private heartbeat() {
    ...
    this.emit(CbEvents.ONWSCONNECTSUCCESS,"" as any)
    ...
    const callback = () => {
      ...
      if (this.ws?.readyState !== (WebSocket?.CONNECTING??0) && this.ws?.readyState !== (WebSocket?.OPEN??1)) {
        this.reconnect();
        return;
      }
