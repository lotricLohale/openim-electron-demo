...
import { CustomMessageType, GroupSessionTypes } from "@/constants/im";
...
const linkWrap = ({
  ...
  return `<span class='link-el' onclick='userClick("${userID}","${
    groupID ?? ""
  }")'>${name}</span>`;
};
  ...
  switch (msg.contentType) {
    ...
    case MessageType.MemberInvited:
      ...
      return t("messageDescription.invitedToGroupMessage", {
        ...
        invitedUser: `${inviteStr}${
          invitedUserList.length > 3 ? `等${invitedUserList.length}人` : ""
        }`,
      });
    ...
    case MessageType.BurnMessageChange:
      ...
      return t("messageDescription.burnReadStatus", {
        status: burnDetails.isPrivate ? "开启" : "关闭",
      });
...
export const formatMessageByType = (message: MessageItem): string => {
  ...
  switch (message.contentType) {
    ...
    case MessageType.CustomMessage:
      ...
      if (customData.customType === CustomMessageType.MassMsg) return "[群发通知]";
      if (customData.customType === CustomMessageType.Call) return "[通话]";
    ...
    case MessageType.MemberKicked:
      ...
      return t("messageDescription.kickInGroupMessage", {
        ...
        kickedUser: `${kickStr}${
          kickdUserList.length > 3 ? `等${kickdUserList.length}人` : ""
        }`,
      });
    case MessageType.MemberQuit:
