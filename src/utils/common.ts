...
import { PictureElem } from "./open_im_sdk/types";
...
export const formatDate = (timestamp: number): any[] => {
  ...
  // const str1 = (currentYear === year ? "" : year + "年") + month + "月" + date + "日";
...
export const parseTime = (timestamp: number, isFull = false) => {
  const arr = formatDate(timestamp);
  return isFull ? arr[3] + " " + arr[4] : arr[4];
};

export const parseLatestTime = (ltime: number): string => {
...
export const getResized = (el: PictureElem) => {
  let elWidth = el.sourcePicture.width;
  let elHeight = el.sourcePicture.height;

  const maxWidth = 400;
  const maxHeight = 400;
  if (elWidth > elHeight) {
    if (elWidth > maxWidth) {
      elHeight = elHeight * (maxWidth / elWidth);
      elWidth = maxWidth;
    }
  } else {
    if (elHeight > maxHeight) {
      elWidth = elWidth * (maxHeight / elHeight);
      elHeight = maxHeight;
    }
  }
...
export const switchLoginError = (errCode: number) => {
  switch (errCode) {
    case 20001:
