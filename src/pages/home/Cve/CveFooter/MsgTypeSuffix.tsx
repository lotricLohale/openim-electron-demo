...
import { Dropdown, Menu, message, Image as AntdImage, Spin, Progress, Tooltip } from "antd";
...
import send_icon_disabled from "@/assets/images/send_icon_disabled.png";
...
type MsgTypeSuffixProps = {
  ...
  sendMsg: (nMsg: string, type: MessageType, uid?: string, gid?: string, showNotification?: boolean, delPath?: string) => void;
  enterSend: () => void;
};

const MsgTypeSuffix: FC<MsgTypeSuffixProps> = ({ disabled, choseCard, faceClick, sendMsg, enterSend }, ref) => {
  ...
  const imgMsg = async (file: RcFile, url: string) => {
    ...
    if (window.electron) {
      ...
      snpPath = catchPath + `${window.electron.isWin ? "\\" : "/"}${uuid("uuid")}.png`;
    ...
    sendMsg(msgStr, MessageType.PICTUREMESSAGE, undefined, undefined, false, snpPath);
  };
  ...
  const changeEmojiStyle = (style: any) => {
    switch (style) {
      ...
      default: // 获取本地表情包
        ...
        const userId = JSON.parse(localStorage.getItem("IMuserID")!);
        // 获取当前用户的表情包
  ...
  const openElecDialog = async (type: string) => {
    ...
    if (res) {
      ...
      const suffix = res[0].slice(res[0].lastIndexOf(".") + 1);
      ...
      if (type === "pic" || (imageSuffixs.includes(suffix) && type === "file")) {
        ...
        const catchPath = window.electron.getCachePath();
        snpPath = catchPath + res[0].slice(nameIdx - 1, suffixIdx) + "png";
  ...
  return (
    <div className="suffix_container">
      ...
      <Tooltip title="发送">
        <img onClick={enterSend} src={disabled ? send_icon_disabled : send_icon} />
      </Tooltip>
