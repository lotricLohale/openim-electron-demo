...
export const GroupParticipantRender = ({ participant, isVideo = true }: ParticipantProps) => {
  ...
  const VideoCpm = (
    <>
      {!cameraPublication?.track || isVideoMuted ? (
      ...
      <div className="remote_info">
        ...
        <img src={!isAudioMuted ? rtc_voice_on : rtc_voice_off} />
      </div>
    </>
  );
  ...
  const AudioCpm = (
    <>
      ...
      <img className="icon" src={!isAudioMuted ? rtc_voice_on : rtc_voice_off} />
    </>
  );
  ...
  return (
    <div className={`par_con ${isSpeaking ? "speaking" : ""}`}>
      ...
      {microphonePublication?.track && <AudioRenderer track={microphonePublication?.track} isLocal={isLocal} />}
    </div>
  );
};
