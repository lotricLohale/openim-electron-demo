...
import { TO_ASSIGN_CVE, UPDATE_FRIEND_CARD } from "../../../constants/events";
...
const UserCard: FC<UserCardProps> = ({ draggableCardVisible, info, close, type }) => {
  ...
  const clickBtn = (isSendMsg = false) => {
    if (isSendMsg) {
      ...
      setTimeout(() => {
        events.emit(TO_ASSIGN_CVE, info.userID, SessionType.Single);
        close();
      });
    } else {
      setStep("send");
  ...
  const updateComment = () => {
    im.setFriendRemark({ toUserID: info.userID!, remark: drft })
      .then((res) => {
        ...
        events.emit(UPDATE_FRIEND_CARD, info);
        message.success(t("ModifySuc"));
      })
      .catch((err) => message.error(t("ModifyFailed")));
  };
    ...
    <>
      <div className="left_info">
        ...
        <div className="left_info_icon">
          ...
          {isFriend && <img onClick={delComfirm} style={{ marginLeft: "8px" }} width={18} src={del_card} />}
        </div>
      </div>
