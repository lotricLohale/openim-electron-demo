...
const Captcha: FC<CaptchaProps> = forwardRef((props, ref) => {
  const { type = "auto", onCancel = noop, onSuccess, onFail = noop, path, className, style, checkUrl, getUrl, checkFuc } = props;
  ...
  const fetch = async () => {
    ...
    const repData = await picture(getUrl ?? `${path}/captcha/get`, {
