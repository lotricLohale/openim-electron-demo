  ...
  pasteLint = (e: React.ClipboardEvent<HTMLInputElement>) => {
    ...
    imageItems.map((item: DataTransferItem) => {
      ...
      reader.onload = () => {
        ...
        image.setAttribute("ondblclick", `screenshotPreview("${reader.result}")`);
    ...
    const files = [] as File[];
    [...clp.items].map((item: DataTransferItem) => {
      const file = item.getAsFile();
      if (!item.type.includes("image") && file) {
        files.push(file);
      }
    });
    // if (files.length > 0) {
    //   let fileNames = "";
    //   files.map((file, idx) => (fileNames += file.name + (idx === files.length - 1 ? "" : ",")));
    //   modal.confirm({
    //     title: "发送文件",
    //     content: `确认发送文件[${fileNames}]给${curCve.showName}?`,
    //     okText: "确认",
    //     cancelText: "取消",
    //     onOk: () => {
    //       files.map((file) => suffixRef.current.copy2Send(file));
    //     },
    //   });
    // }
  };
